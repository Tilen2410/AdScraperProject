...
class AdspiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for apartment in apartments:
            ...
            yield response.follow(apartment_page_url, callback=self.parse_apartment_page, meta={'playwright': True})
    ...
    def parse_apartment_page(self, response):
        ...
        #title2 = response.xpath("//div[@class='property-title']/h1/span/span[contains(@class, 'name') and contains(@class, 'ng-binding')]/text()").get()         btn-paging-pn icof icon-arr-right paging-next
        image1 = response.css('.ng-isolate-scope .ob-c-carousel__content img::attr(src)').get()
        #image2 = response.xpath("//div[@class='ng-isolate-scope']/div[@class='ob-c-carousel__content']/div[1]/img/@src").get()
        """
        yield {
            'title': title1,
            'image_url': image1,
